...
## 关于如何区分自身坐标轴变换与世界坐标轴变换

对于自身坐标轴变换，比如我现在有$T_{B}^{W}$，在该局部的坐标轴变换时，例如沿自身x轴旋转$+\pi/2$，应该右乘一个旋转矩阵得到$T_{B_1}^{W}=T_{B}^W \times R_x(\pi/2)$，至于为什么右乘，这是因为我们在变换的时候相当于先由W变换到B再变换到$B_1$，应该先施加$T_B^W$，再乘$R_x(+\pi/2)$

对于世界坐标轴的变换则需要左乘，也是用上面的解释。
